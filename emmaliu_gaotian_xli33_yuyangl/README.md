...
In Project #1, we get 5,000 tweets but the number of tweets with "geo" is not enough. To get more geo information, we get all tweets in last 7 days in Amman area. There are about 29,000 tweets in total and about 160 tweets include "geo". We use coordinates in "geo" to do clustering. The most classic clustering algorithm is k-means. In this project, we implement k-means++ to better initialize the cluster centers. When k is set to 3, we get the result as follows.
